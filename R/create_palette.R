...
#'
#' @export
create_palette <- function(name, authors, colors=NULL, type=NULL, github_user=NULL, description=Sys.Date(), keywords, date) {
  palette_data <- drop_null(list(name = jsonlite::unbox(name), authors = authors, colors = colors, type = jsonlite::unbox(type), github_user = jsonlite::unbox(github_user), description = jsonlite::unbox(description), keywords = keywords, date = jsonlite::unbox(date)))
